...
pi-web UI layers (jmfederico + agegr) moved out to `~/piagent/pi-web/` (own mise task namespace `piweb:*`, registered via `~/.config/mise/conf.d/pi-web-layers.toml`). They source `tasks/pi/_docker_flags` by absolute path.
